...
app.config["SQLALCHEMY_DATABASE_URI"] = os.environ.get('DATABASE_URL') or "postgresql://localhost/sindhgirHW4db" # TODO 364: You should edit this to correspond to the database name YOURUNIQNAMEHW4db and create the database of that name (with whatever your uniqname is; for example, my database would be jczettaHW4db). You may also need to edit the database URL further if your computer requires a password for you to run this.
...
search_gifs = db.Table('search_gifs',db.Column('searchterms_id', db.Integer, db.ForeignKey('searchterms.id')), db.Column('gifs_id',db.Integer, db.ForeignKey('gifs.id')))

user_collection = db.Table('user_collection',db.Column('gifs_id', db.Integer, db.ForeignKey('gifs.id')),db.Column('personalgifcollections_id',db.Integer, db.ForeignKey('personalgifcollections.id')))
...
class User(UserMixin, db.Model):
    ...
    user_gifcollections = db.relationship('PersonalGifCollection', backref='User')
...
def get_or_create_search_term(term):
    ...
    if search_term:
        ...
        search_item = get_gifs_from_giphy(term)
        for x in search_item["data"]: 
            title = x["title"]
            url = x["embed_url"]
            y = get_or_create_gif(title, url)
            search_term.gifs.append(y)
...
def get_or_create_collection(name, current_user, gif_list=[]):
    ...
    gif_collection = db.session.query(PersonalGifCollection).filter_by(name=name, users_id=current_user.id).first()
    if gif_collection: 
        return gif_collection
    else:
        gif_collection = PersonalGifCollection(name=name, users_id=current_user.id)
        for gif in gif_list: 
            gif_collection.gifs.append(gif)
        db.session.add(gif_collection)
        db.session.commit()
        return gif_collection
...
def create_collection():
    ...
    if form.validate_on_submit(): 
        ...
        for x in form.gif_picks.data: 
            gif_list.append(get_gif_by_id(x))
